...
export function useTmdbSearch() {
  ...
  async function searchTmdb(query) {
    ...
    try {
      const res = await fetch(`${TMDB_BASE_URL}/search/multi?api_key=${TMDB_API_KEY}&language=ru-RU&query=${encodeURIComponent(query)}`)
  ...
  async function fetchTmdbDetails(id, mediaType) {
    ...
    try {
      const res = await fetch(`${TMDB_BASE_URL}/${mediaType}/${id}?api_key=${TMDB_API_KEY}&language=ru-RU`)
      if (!res.ok) throw new Error(`TMDB error: ${res.status}`)
