...
func (r *postgresReviewRepository) Create(ctx context.Context, userID int64, req model.CreateReviewRequest) (*model.Review, error) {
	...
	err := r.db.QueryRow(ctx, `
		INSERT INTO reviews (user_id, title, content_type, status, rating, notes, poster_url, created_at, updated_at)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
		RETURNING id, user_id, title, content_type, status, rating, notes, poster_url, created_at, updated_at
	`, userID, req.Title, req.ContentType, req.Status, req.Rating, req.Notes, req.PosterURL, now, now).
		Scan(&review.ID, &review.UserID, &review.Title, &review.ContentType, &review.Status,
			&rating, &review.Notes, &review.PosterURL, &review.CreatedAt, &review.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func (r *postgresReviewRepository) GetAllByUserID(ctx context.Context, userID int64) ([]*model.Review, error) {
	rows, err := r.db.Query(ctx, `
		SELECT id, user_id, title, content_type, status, rating, notes, poster_url, created_at, updated_at
...
func (r *postgresReviewRepository) GetByID(ctx context.Context, id, userID int64) (*model.Review, error) {
	...
	`, id, userID).Scan(&review.ID, &review.UserID, &review.Title, &review.ContentType, &review.Status,
		&rating, &review.Notes, &review.PosterURL, &review.CreatedAt, &review.UpdatedAt)
	if err != nil {
...
func (r *postgresReviewRepository) Update(ctx context.Context, id, userID int64, req model.UpdateReviewRequest) (*model.Review, error) {
	...
	var rating *int16
	err := r.db.QueryRow(ctx, `
		UPDATE reviews SET title=$1, content_type=$2, status=$3, rating=$4, notes=$5, poster_url=$6, updated_at=$7
		WHERE id=$8 AND user_id=$9
		RETURNING id, user_id, title, content_type, status, rating, notes, poster_url, created_at, updated_at
	`, req.Title, req.ContentType, req.Status, req.Rating, req.Notes, req.PosterURL, time.Now(), id, userID).
		Scan(&review.ID, &review.UserID, &review.Title, &review.ContentType, &review.Status,
			&rating, &review.Notes, &review.PosterURL, &review.CreatedAt, &review.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
